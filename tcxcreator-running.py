...
def createTcx(basename):
    ...
    def createTcxEntry(gpx, hrm_data, distance):
        ...
        if CADENSE:
            e = ElementTree.SubElement(tp, 'Extensions')
            t = ElementTree.SubElement(e, 'TPX', {'xmlns': 'http://www.garmin.com/xmlschemas/ActivityExtension/v2'})
            c = ElementTree.SubElement(t, 'RunCadence')
            c.text = str(hrm_data[2])
        return distance
